...
export default function Home() {
  ...
  const [clients, setClients] = useState<Client[]>([
    { id: '1', name: '田中 太郎', address: '東京都渋谷区...', careLevel: '要介護1' },
    { id: '2', name: '佐藤 花子', address: '東京都新宿区...', careLevel: '要支援2' },
    { id: '3', name: '鈴木 一郎', address: '東京都港区...', careLevel: '要介護3' },
  ]);
  ...
  const handleDeleteScheduleType = (id: string) => {
    // Prevent deleting default types if necessary, but for now allow all
      ...
      <header className="bg-white shadow-sm border-b border-slate-200">
        <div className="px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 text-sm text-slate-500">
            ...
            <span className="text-xs bg-slate-100 px-2 py-0.5 rounded text-slate-400">v0.1.8</span>
          </div>
        </div>
          ...
          <>
            <div className="flex-grow bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden flex flex-col">
              <div className="p-4 border-b border-slate-100 flex justify-between items-center">
                <h2 className="font-semibold text-slate-700">
                  {selectedClientId ? `${clients.find(c => c.id === selectedClientId)?.name} 様のスケジュール` : 'スケジュール一覧'}
                </h2>
            ...
            <div className="w-80 h-full flex flex-col">
              <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-4 h-full flex flex-col">
                <h2 className="font-semibold text-slate-700 mb-4">利用者一覧</h2>
